      ...
      services.opentracker = lib.mkIf cfg.opentracker.enable {
        ...
        package = pkgs.opentracker.overrideAttrs (
          final: prev: {
            ...
            makeFlags =
              prev.makeFlags ++ lib.optionals cfg.opentracker.supportReverseProxy [ "FEATURES=-DWANT_IP_FROM_PROXY" ];
          }
        );
        # allows setting up a reverse proxy
        extraOptions = "-f ${
          pkgs.writeText "opentracker-config" (
            lib.strings.concatLines (lib.optionals cfg.opentracker.supportReverseProxy [ "access.proxy 127.0.0.1" ])
          )
        }";
      };
